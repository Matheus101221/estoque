...
async function fetchSheetTab(sheetName) {
  ...
  // O parâmetro "_ts" muda a cada chamada só para evitar que o navegador
  // (ou algum proxy no meio do caminho) sirva uma resposta antiga em cache.
  const url = `https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(sheetName)}&_ts=${Date.now()}`;
  ...
  // Preenche modelo/ano/cor quando vêm em branco, reaproveitando o valor da
  // linha anterior. Isso resolve o caso comum de células mescladas na planilha,
  // onde o nome do modelo/ano/cor só é digitado na primeira linha de um grupo
  // de motos iguais e as linhas seguintes ficam em branco nessas colunas —
  // sem isso, essas linhas eram descartadas por não terem "modelo" preenchido.
  let lastModelo = "", lastAno = "", lastCor = "";
  for (const item of items) {
    const temModelo = item.modelo && String(item.modelo).trim() !== "";
    if (temModelo) {
      lastModelo = item.modelo;
      lastAno = item.ano || lastAno;
      lastCor = item.cor || lastCor;
    } else if (item.chassi && String(item.chassi).trim() !== "") {
      // Linha sem modelo mas com chassi preenchido: é uma unidade de verdade,
      // só que herdando os dados do grupo anterior.
      item.modelo = lastModelo;
      if (!item.ano) item.ano = lastAno;
      if (!item.cor) item.cor = lastCor;
    }
  }
  ...
  .filter(item => {
    ...
    return !(st.includes("vendid") || st.includes("sold"));
  })
  .map(item => {
...
function renderContent() {
  ...
  for (const [modelo, units] of groups) {
    ...
    units.forEach(unit => grid.appendChild(renderUnitCard(unit, { min, media, max }, tabName)));
    container.appendChild(section);
  }
}

function renderUnitCard(unit, modelStats, tabName) {
  ...
  card.innerHTML = `
    ...
    <div class="unit-body">
      ...
      <div class="card-actions">
        <button class="card-action mark-sold" type="button">MARCAR COMO VENDIDA</button>
      </div>
    </div>
  `;
  ...
  if (soldBtn) soldBtn.addEventListener("click", () => markAsSold(unit, tabName, card));

  return card;
}

function showSoldModal(unit) {
  return new Promise(resolve => {
    const overlay = document.getElementById("sold-modal");
    const text = document.getElementById("sold-modal-text");
    const confirmBtn = document.getElementById("sold-modal-confirm");
    const cancelBtn = document.getElementById("sold-modal-cancel");

    text.textContent = `${unit.modelo} — chassi ${unit.chassi}. Ela vai sumir do site pra todo mundo assim que você confirmar.`;
...
async function markAsSold(unit, tabName, card) {
  if (!CONFIG.APPS_SCRIPT_URL || !CONFIG.APPS_SCRIPT_URL.trim()) {
    showToast("Falta configurar o APPS_SCRIPT_URL no config.js");
    return;
  }
  if (!unit.chassi) {
    showToast("Essa unidade não tem chassi cadastrado, não dá pra marcar");
    return;
  }

  const confirmed = await showSoldModal(unit);
  if (!confirmed) return;

  const btn = card.querySelector(".mark-sold");
  ...
  try {
    const res = await fetch(CONFIG.APPS_SCRIPT_URL, {
      ...
      body: JSON.stringify({ sheetName: tabName, chassi: unit.chassi }),
    });
    ...
    try {
      ...
      throw new Error(`Resposta não veio em JSON (HTTP ${res.status}). Provavelmente o Apps Script não está implantado como "Qualquer pessoa" ou a URL não termina em /exec.`);
    }
    if (!data.ok) throw new Error(data.error || "Falha ao marcar como vendida");

    state.dataByTab[tabName] = (state.dataByTab[tabName] || []).filter(u => u.chassi !== unit.chassi);
    showToast("Marcada como vendida");
    renderTabs();
    renderContent();
  } catch (err) {
    console.error(err);
    showToast(err.message || "Não consegui marcar como vendida. Tente de novo.");
